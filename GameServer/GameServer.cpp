...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message) 
	{
		...
		case WM_CLOSE:
			if ( gObjTotalUser > 0 )
			{
				MessageBox(ghWnd, "Players are already connected to GameServer \nPlease disconnect all players first", "Close", MB_OK|MB_ICONERROR|MB_TOPMOST);
			}
			else
			if (MessageBox(ghWnd, "Do you want to close eGameServer?", "Close", MB_YESNO|MB_APPLMODAL|MB_TOPMOST) == IDYES)
			{
