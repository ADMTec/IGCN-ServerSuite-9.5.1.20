...
void CServerInfo::Load(std::string file)
{
	...
	if (!hFile)
	{
		MessageBoxA(0, "ServerInfo.bmd is missing", "ERROR", MB_OK |MB_TOPMOST);
		ExitProcess(0);
	}
	...
	GetPrivateProfileStringA("Main", "Version", "1.05.24", szTemp, 8, filedec);
	...
	GetPrivateProfileStringA("Main", "Serial", "PowerByDvTrong", szTemp, 17, filedec);
