...
void CConServ::LoadServerList(LPSTR szFile)
{
	...
	{
		MessageBoxA(0,"CConServer::LoadServerList() error","CRITICAL ERROR",MB_OK|MB_ICONERROR|MB_TOPMOST);
		ExitProcess(1);
		return;
	}
...
void CConServ::LoadNewsFile(LPSTR szFile)
{
	for(int i = 0; i < 10; i++ )
	{
	 ...
	 if ( (SMDFile = fopen(szFile, "r")) == NULL )
		 {
			MessageBoxA(0,"CQuestManager::LoadFile() error","CRITICAL ERROR",MB_OK|MB_ICONERROR|MB_TOPMOST);
			ExitProcess(1);
			return;
		 }
		SMDToken Token;
