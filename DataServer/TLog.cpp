...
void TLog::Add(char * szLog, ...)
{
	...
	today = localtime(&ltime);
		...
		try 
			...
			{ 
				sprintf(szLogBuff, "[%.8s] [ANTI_CRASH_HACK] [TLog::Add] ", asctime(today) + 11); strcat(szLogBuff, szLog); 
...
void TLog::AddF(char * szLog, ...)
{
	...
	va_start(lpArguments, szLog);
		...
		try 
			...
			{ 
				sprintf(szLogBuff, "[%.8s] [ANTI_CRASH_HACK] [TLog::AddF] ", asctime(today) + 11); strcat(szLogBuff, szLog); 
				LogTextAdd(TColor::Red, szLogBuff, strlen(szLogBuff)); 
				return; 
			} 
...
void TLog::AddL(TColor::T tColor, char * szLog, ...)
{
	...
	va_start(lpArguments, szLog);
	//vsprintf(szArgBuff, outChar, lpArguments);// Add Friend Crash
