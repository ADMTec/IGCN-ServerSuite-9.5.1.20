                     ...
                     int       nCmdShow)
{
	...
	GetPrivateProfileString("AutoUpdate", "Version", "1.05.24", g_ClientVersion, 9, ".\\ConnectServer.ini");
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case ID_RELOAD_AUTOUPDATE:
			GetPrivateProfileStringA("AutoUpdate", "Version", "1.05.24", g_ClientVersion, 9, ".\\ConnectServer.ini");
	...
	case WM_DESTROY:
		...
		break;
	case WM_CLOSE:
			if ( MessageBox(NULL, "Are you sure to Close?", "Confirmation", MB_YESNO|MB_ICONQUESTION| MB_TOPMOST) == IDYES )
				DestroyWindow(hWnd);
			break;
	default:
		return DefWindowProc(hWnd, message, wParam, lParam);
